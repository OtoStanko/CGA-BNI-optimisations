...
public class DataGen {
                    ...
                    for (j = 0; j < i; j++) {
                        ...
                        while (true) {
                            ...
                            if (!Utils.checkExistInteraction(temp, numofedges, inatemp)) {
                                /*2.0d * numofedges*/
                                prob += (double) degrees[j] / (oldTotalDegrees - degreeIgnore);
                                existing = false;
                                break;
                            }
    ...
    public static boolean generate_KO_Attractors(NetData netD, int noStates, String path) throws FileNotFoundException {
        ...
        ArrayList<String> wtAtts = new ArrayList<>();
        ...
        for (Attractor allAttractor : AllAttractors) {
            if (Utils.exist_inverted(false, dones)) break;

            if (allAttractor.Length > 1) {
                continue;
            }

            boolean[][] wt_state = Utils.convert_2DBool(allAttractor.States.get(0));
            boolean exist_KOFixedPointAtt = false;

            for (int pertG = 0; pertG < numGenes; pertG++) {
                if (dones[pertG]) continue;
            ...
            if (exist_KOFixedPointAtt) {
                wtAtts.add(allAttractor.States.get(0));
            }
        }
        ...
        if(Utils.exist_inverted(false, dones)) {
    ...
    public static void createRBNs(int numNodes, int numNetworks) throws IOException {
        ...
        while( cnt < numNetworks ) {
            String netName = "Size" + numNodes + "_RBN" + (cnt + START_INDEX_NETWORK);
